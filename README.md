...
Extracts JAR file contents using `System.IO.Compression.ZipFile`
Analyzes internal file names and paths via regex
Reads content of `.class`, `.json`, and `MANIFEST.MF` files
Searches for suspicious patterns (class/file names) and strings (hardcoded cheat-related text)
Detects fullwidth Unicode strings (e.g. `Ａｕｔｏ Ｃｒｙｓｔａｌ`) used to hide cheat labels
Also scans nested JARs embedded in `META-INF/jars/`
---
